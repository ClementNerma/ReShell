                ...
                |at, Args { path }, ArgsAt { path: path_at, .. }, ctx| {
                    let path = match path {
                        None => std::env::current_dir()
                            .map_err(|err| ctx.error(at, format!("Failed to get path to current directory: {err}")))?,

                        Some(path) => {
                            let path = PathBuf::from(path);

                            if !path.is_dir() {
                                return Err(ctx.error(path_at.unwrap(), format!("Directory not found at path: {}", path.display())))
                            }
                    ...
                    let read_dir = fs::read_dir(&path).map_err(|err| {
                        ctx.error(at, format!("Failed to read directory: {err}"))
                    })?;
                    ...
                    for item in read_dir {
                        ...
                        let metadata = fs::symlink_metadata(item.path()).map_err(|err| {
                            ctx.error(at, format!("Failed to get metadata from item '{}': {err}", item.path().display()))
                        })?;
                        ...
                        items.push(LsItem {
                            ...
                            item_type: if metadata.is_dir() {
                                ...
                                LsItemType::File { size: metadata.len() }
                            } else if metadata.is_symlink() {
                                LsItemType::Symlink {
                                    target: fs::read_link(item.path())
                                        .map_err(|err| {
                                            ctx.error(at, format!("Failed to read symlink target from'{}': {err}", item.path().display()))
                                        })?
                                }
                            } else {
                                LsItemType::Unknown
                    ...
                    items.sort_by(|a, b| a.item_type.cmp(&b.item_type).then_with(|| a.name.cmp(&b.name)));
                    ...
                    for item in items {
                        ...
                        match item_type {
                            ...
                            LsItemType::Symlink { target } => {
                                println!("{} {}", name.bright_magenta(), format!("-> {}", target.to_string_lossy()).bright_yellow());
                            }
